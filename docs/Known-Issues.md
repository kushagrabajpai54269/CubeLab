...
- **Phase C, Subsystem 5 (Last-Layer Orientation / 2-look OLL)** — resolved: complete (2026-09-04), D-050. See Bugs below for the `dCornerTwist` defect found and fixed during its derivation.
...
- **[Resolved, D-050]** `dCornerTwist` slot-invariance bug, found during Phase C, Subsystem 5 (Last-Layer Orientation) derivation (2026-09-04): a helper meant to detect a D-layer corner's twist state by reusing `pieces.ts`'s `cornerOrientation` directly against the piece's own home-identity orientation was found, via direct empirical testing, to be **not slot-invariant** — a single pure D turn (which physically only permutes D-layer corners, never twists them) registered a nonzero "twist" at slots other than a piece's own home slot. Root-caused and removed entirely (not patched) in favor of the already-correct, slot-local `isLastLayerCornerOriented` boolean, which is all the corner-orientation search and orchestration actually need. Never shipped in any release build — caught during the same subsystem's own derivation/testing pass, before the corner-orientation algorithm was accepted. Full account in the Decision Log (D-050) and Development Journal (Entry 22).

## Performance Bottlenecks
- **Phase C, Subsystem 4 (Second-Layer Edges) move-count.** `solveSecondLayerEdges` produces visibly longer move sequences than Subsystems 2/3 (max ~850 moves across 1500 seeded fuzz trials, vs. corners' under 300) — each individual insertion is 8 moves (vs. corners' 3), and the greedy least-disturbance target-selection strategy (D-049) that fixed a real termination defect isn't itself move-count-optimized. Correctness and termination are fully verified; this is an accepted trade-off, not a defect (move-count efficiency was never a stated goal for this phase). Worth revisiting once Phase D's step-through move animation makes a long sequence visibly slow to watch for a user.
- **Phase C, Subsystem 5 (Last-Layer Orientation) corner-step solve-time search.** `solveLastLayerCornerOrientation` runs a small bounded round-level search (branching 4, depth <=8) at solve time rather than a precomputed alignment table — milliseconds per call in testing, not a current concern, but flagged (D-050) as worth precomputing/caching if Phase D's real-time interactive solver ever makes per-call solve latency more visible to users.
_No other performance testing has been done yet — not relevant at current scale (a single 2D input page); revisit once Phase D's 3D rendering and Phase E/F's larger data sets (case tables, algorithm search) exist._
